...
def food(request, id):
    fruit = get_object_or_404(FruitTable, id=id, user=request.user)
    foods = FruitTable.objects.filter(user=request.user)
    result = FruitImg.objects.get(id=id)
    ...
    return render(request, 'fruit/food.html', context)
